...
class Grafo():
    ...
    def insert_arista(self, vertice, criterio_vertice=None, criterio_arista='vertice'):
        origen = self.search_vertice(vertice, criterio_vertice)
        if origen is not None:
            for destino in self.vertices:
                if destino != origen:
                    peso = randint(1, 7) 
                    self.get_element_by_index(origen)[1].insert(Arista(destino[0], peso), criterio_arista)
                    if not self.dirigido:
                        self.get_element_by_index(destino)[1].insert(Arista(vertice, peso), criterio_arista)
